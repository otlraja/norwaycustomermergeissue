...
import 'package:norwayfinalcustomer/NotificationClasses/Distance.dart';
...
class _MyAppState extends State<Taxi_Map> {
  ...
  void showpool(){
    ...
    userdataref = database.reference().child('DriverLatLong');
    userdataref.once().then((DataSnapshot snapshot) {
      if (snapshot.value != null) {
        Map<String, dynamic> mapOfMaps = Map.from(snapshot.value);
        for( var i in mapOfMaps.values){
          var lat=i['latitude'];
          var long=i['longitude'];
          var restaurant_dist=Distance().distance1( current.latitude, current.longitude,lat,long);
          if(restaurant_dist <= 500){
            addpool(lat,long);
          }
        }
      }
  ...
  Future<void> ServiceSnapshot() {
    final FirebaseDatabase database = FirebaseDatabase.instance;
    ServicesRef = database.reference().child('ServiceType');
                ...
                setState(() {
                  ...
                  if (SplashTest.toke.length == 1) {
                    postRequest()
                        .makePostRequest(
                        "Ride Request",
                        "" + UserData.Name + "Requested for Ride",
      ...
      mapOfMapss.values.forEach( (value2) {
        setState(() {
          ...
          if(SplashTest.toke.length == 1) {
            postRequest()
                .makePostRequest(
                "Ride Cancelled", ""+UserData.Name+"Restaurant han cancle the Ride", SplashTest.toke[0].toString());
                                            ...
                                            final TimeOfDay pickedS = await showTimePicker(
                                              ...
                                              return Theme(
                                                data: ThemeData.light().copyWith(
                                                  ...
                                                  accentColor: AppColors.lightgreenColor,
                                                  colorScheme: ColorScheme.light(primary: AppColors.lightgreenColor),
                      ...
                      onTap: (){
                        setState(() {
                          ...
                          if(selectedServiceBudget==0){
                            ...
                            if(selectTime=='select date'){
                              Fluttertoast.showToast(
                                  msg: "Please Select Date to Shedule Ride.",
                                  toastLength: Toast.LENGTH_SHORT,
                                  gravity: ToastGravity.CENTER,
                                  timeInSecForIosWeb: 2,
                                  backgroundColor: Colors.red,
                                  textColor: Colors.white,
                                  fontSize: 16.0);
                            }else if(selectDate=='select time'){
                              Fluttertoast.showToast(
                                  msg: "Please Select Time to Shedule Ride.",
                                  toastLength: Toast.LENGTH_SHORT,
                                  gravity: ToastGravity.CENTER,
                                  timeInSecForIosWeb: 2,
                                  backgroundColor: Colors.red,
...
void  getSelectedpayment(String paymentflag){
  ...
  Future<void> SendRequest() {
    ...
    if(sheduleflage==false){
      ...
      Fluttertoast.showToast(
          msg: "Your Request has been sent you can see in Ongoing Orders.",
          toastLength: Toast.LENGTH_SHORT,
          gravity: ToastGravity.CENTER,
          timeInSecForIosWeb: 3,
          backgroundColor: Colors.red,
